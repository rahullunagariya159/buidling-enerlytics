...
function AddCard(props) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (checkCName && isValidCardNumber && isValidCardExpiry && isValidCardCvv) {
      ...
      getPaymentData({ 'selectedCard': selectedCard, 'cardType': cardType, 'name': cardName, 'error': error, 'paymentMethod': paymentMethod, 'token': token });
    }
  };
      ...
      <form onSubmit={handleSubmit}>
        <div className="credit-card-detal">
          <div className="RELIABLE-grid">
            <h1 className="SECURE-tit"><span className="bord-SECUR">SECUR</span>E AND RELIABLE</h1>
          ...
          <div className="Card-drid-24px">
            <div className="Card-type-grid">
              ...
              <label className="dropdown awdw">
                ...
                <ul className="dd-menu ">
                  <li className="card-bord-none" onClick={() => selectedCardFn('credit', 'Credit card')}>Credit card</li>
                  <li className="card-bord-none" onClick={() => selectedCardFn('debit', 'Debit card')}>Debit card</li>
                </ul>
              </label>
            </div>
            <div className="flex-card-24px">
              <div className="Card-type-grid card-number">
                ...
                {cardType === 'mastercard' &&
                  (
                    <img src="assets/img/Add-credit-card-details/master-credit-card.png" className="vicaa-ic" alt="" />
                  )
                }
                {cardType === 'visa' &&
                  (
                    <img src="assets/img/Add-credit-card-details/visa-credit-card.png" className="vicaa-ic" alt="" />
                  )
                }
            ...
            <div className="flex-card-24px">
              <div className="Card-type-grid">
                ...
                <input id="cardName" type="text" placeholder="Enter Name on the card" onChange={(e) => setCardName(e.target.value)} className="Card-type-input" />
              </div>
          ...
          <div className="pay-back-flex p-0">
            ...
            {props.addCardClicked ? (
              <button className="Pay-btn loading-button" type='submit'><i class="fa fa-spinner fa-spin"></i> Add</button>
            ) : (
              <button className="Pay-btn" type='submit'>Add</button>
            )}

            <a className="Back-btn" data-bs-dismiss="modal" data-bs-toggle="modal" data-bs-target="#PROMOCODE">Close</a>
          </div>
        </div>
      </form>
    </div>
